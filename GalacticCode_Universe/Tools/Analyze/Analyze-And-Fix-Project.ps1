...
if (-not $ProjectPath) {
    # Try to find the repo root by looking for a marker file (e.g. README.md, .git, package.json)
    $here = (Get-Location).Path
    $root = $here
    while ($true) {
        if (Test-Path (Join-Path $root ".git") -or Test-Path (Join-Path $root "README.md") -or Test-Path (Join-Path $root "package.json")) {
            break
        }
